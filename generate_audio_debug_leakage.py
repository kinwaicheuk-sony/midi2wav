      ...
      for part_number, instrument in enumerate(midi_data.instruments):
        ...
        if midi_program in allowed_midi_program:
          ...
          save_wav(audio,
                   os.path.join(
                     output_dir,
                     f'{part_number}_{filename}_fluidsynth.wav'),
                   SAMPLE_RATE)
...
def reset_tensorflow():
    # For TensorFlow 1.x and 2.x (compatibility mode)
...
def main():
  ...
  if args.midi_dir and args.midi_path:
    ...
    del output_dict, midi_data, conditioning_df
    gc.collect()      
